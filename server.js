...
app.post('/api/computadores', (req, res) => {
  ...
  db.run(
    'INSERT INTO computadores (unidade_id, setor, patrimonio, quantidade) VALUES (?, ?, ?, ?)',
    [unidade_id, setor, patrimonio, quantidade || 1],
    function (err) {
      if (err) return res.status(500).json(err);
      res.json({ id: this.lastID });
    }
...
// Atualizar computador (editar campos e status otimizado)
app.put('/api/computadores/:id', (req, res) => {
  const { setor, patrimonio, quantidade, otimizado } = req.body;
